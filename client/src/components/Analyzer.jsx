...
const Analyzer = () => {
  ...
  const handleAnalyze = async (code, language = "javascript") => {
    ...
    setLoading(true);
    try {
      const apiBaseUrl = import.meta.env.VITE_API_URL || "https://time-complexity-analyzer-production.up.railway.app";
      const res = await axios.post(`${apiBaseUrl}/api/analyze`, {
        code,
        language,
      });
      setResult({
        complexity: res.data.complexity || "Unknown",
        explanation: res.data.explanation || "No explanation provided."
      });
    } catch (err) {
      setResult({
        complexity: "Error",
        explanation: "Error analyzing code: " + (err.response?.data?.error || err.message)
      });
    } finally {
      setLoading(false);
    }
