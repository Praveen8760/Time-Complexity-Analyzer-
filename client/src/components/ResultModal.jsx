            ...
            {loading ? (
              ...
              <div className="space-y-4 max-h-64 overflow-y-auto bg-black/20 p-4 rounded-lg border border-white/10 text-sm font-mono text-white">
                {result && result.complexity && (
                  <div className="text-blue-400 font-semibold text-base bg-white/5 px-3 py-2 rounded-lg shadow-sm">
                    🧠 Big-O Complexity: {result.complexity}
                  </div>
                )}
                {result && result.explanation && (
                  <div className="mt-4">
                    <span className="text-pink-400 font-bold block mb-1">📘 Explanation:</span>
                    <span className="text-white whitespace-pre-line leading-relaxed">
                      {result.explanation}
                    </span>
                  </div>
                )}
                {(!result || (!result.complexity && !result.explanation)) && (
                  <div className="text-red-400">
                    No analysis data found or invalid format returned.
                  </div>
                )}
              </div>
            )}
